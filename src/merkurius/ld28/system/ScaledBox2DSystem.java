 ...
 * @author Alexandre
 */
public class ScaledBox2DSystem extends EntityProcessingSystem implements PhysicsSystem {
	...
	public ScaledBox2DSystem(float scale) {
		this();
		this.scale = scale;
	}
	...
	protected void updateBody(Entity e) {
		...
		bodyMapper.get(e).getBody().setTransform( 
				transformMapper.get(e).getPosition2().cpy().div(scale), 
				transformMapper.get(e).rotation * MathUtils.degreesToRadians 
			);
		
		// Updating speed
		if( velocityMapper.has(e) ) { 
			bodyMapper.get(e).getBody().setLinearVelocity(velocityMapper.get(e).speed.cpy().div(scale));
	...
	private void updateEntitiesAfterWorldUpdate() {
		...
		for (int i = 0, s = getActives().size(); s > i; i++) {
			...
			transformMapper.get(e).setPosition(bodyMapper.get(e).getBody().getPosition().cpy().mul(scale));
			...
			if( velocityMapper.getSafe(e) != null ) { // adding speed
				velocityMapper.get(e).speed = bodyMapper.get(e).getBody().getLinearVelocity().cpy().mul(scale);
